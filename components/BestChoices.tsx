...
export default function BestChoices() {
    const [interestOptions, setInterestOptions] = React.useState<{ rate: number; position: number }[]>([]);
    React.useEffect(() => {
    ...
    // Use position to find SARON, 5Y, 10Y
...
>
        ...
        <div
          ...
          "
        >
          <RateCard title={t("bestChoices.saronFrom")} value={saron ? (saron.rate * 100).toFixed(2) + "%" : "-"} />
          <RateCard title={t("bestChoices.fiveYearsFrom")} value={fiveY ? (fiveY.rate * 100).toFixed(2) + "%" : "-"} />
          <RateCard title={t("bestChoices.tenYearsFrom")} value={tenY ? (tenY.rate * 100).toFixed(2) + "%" : "-"} />
        </div>
