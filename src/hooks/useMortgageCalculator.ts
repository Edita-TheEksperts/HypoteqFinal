...
export default function useMortgageCalculator() {
  ...
  const [interestOptions, setInterestOptions] = useState<{ label: string; rate: number }[]>([]);
    ...
    fetch("/api/interest-options")
      ...
      .then((data) => {
        if (Array.isArray(data) && data.length > 0 && typeof data[0] === 'object') {
          ...
          setInterestOption(data[0].label);
        }
      });
  }, []);

  const effectiveRate = useMemo(() => {
    const found = interestOptions.find(opt => opt.label === interestOption);
    if (found) return found.rate;
    return 0.01;
  }, [interestOption, interestOptions]);
